...
class cnn:
    ...
    def build(self):
        """:return: A convolutional neural network object"""
        model = Sequential()

        # Order of input shape is different for Tensorflow backend compared to Theano backend
        model.add(Conv2D(filters=32, kernel_size=(3, 3), input_shape=(self.height, self.width, self.channels), activation='relu'))

        # Pooling to reduce size of feature map
